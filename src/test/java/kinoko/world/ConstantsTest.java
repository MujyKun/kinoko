...
public final class ConstantsTest {
    ...
    public void testExpTable() {
        // 1. Define the Legacy Table (Levels 0-199)
        // These are hardcoded to ensure no regression on standard gameplay behavior
        long[] legacyValues = new long[]{
        ...
        // 2. Build the Full Expected Table (Levels 0-999)
        long[] expected = new long[1000];

        // Copy legacy values into the expected array
        System.arraycopy(legacyValues, 0, expected, 0, legacyValues.length);

        // Generate expected values for 200 - 999 based on the new logic in GameConstants
        // Level 200 (Transition)
        expected[200] = (long) ((double) expected[199] * 1.06 + 0.5);

        for (int i = 201; i < 999; i++) {
            if (i < 250) {
                // Phase 1: 200-249 (High Growth - 1.05x)
                expected[i] = Math.round((double) expected[i - 1] * 1.05);
            } else if (i < 300) {
                // Phase 2: 250-299 (Medium Growth - 1.02x)
                expected[i] = Math.round((double) expected[i - 1] * 1.02);
            } else {
                // Phase 3: 300-999 (Consistent Growth - 1.01x)
                expected[i] = Math.round((double) expected[i - 1] * 1.01);
            }
        }

        // Final Level Cap (Max Level requires 0 exp to next)
        expected[999] = 0;

        // 3. Test against GameConstants directly
        Assertions.assertEquals(1000, GameConstants.EXP_TABLE.length, "Table size should be 1000");
        Assertions.assertArrayEquals(expected, GameConstants.EXP_TABLE, "EXP Table values do not match");
    }
}
